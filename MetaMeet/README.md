...
- 음성으로 판정된 녹음은 사용자가 지정한 Gemini 모델로 전송됩니다. Google의 API 데이터 취급 조건과 요금은 사용하는 계정/서비스 설정을 따릅니다. 자체 서버는 없습니다. Meta SDK의 분석/크래시 수집은 설정에서 끕니다.
...
무음 환각 대응을 강화했습니다. WebRTC VAD(aggressive mode 3)로 음성이 감지된 구간만 API에 전달합니다. 기본/저장된 기술 용어 목록은 전사 요청에서 완전히 제거했습니다. Python 바인딩 없이 WebRTC C/C++ 신호 처리만 앱 내부에서 실행합니다. 출처/라이선스는 WebRTC-LICENSE.txt와 Core/Sources/CWebRTCVAD/PROVENANCE.md를 참조하세요.

설정의 음성 처리 속도는 빨리(기본), 중간, 느리게 세 가지입니다. 내부 녹음 구간은 각각 2/5/15초이며 새 회의 시작 때 고정됩니다.
