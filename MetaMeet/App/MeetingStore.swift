        ...
        worker = Task { [weak self] in
            ...
            while !Task.isCancelled {
                ...
                do {
                    ...
                    for attempt in 0..<3 {
                        do {
                            result = try await GeminiClient().transcribe(audio: audio, duration: chunk.duration, model: UserDefaults.standard.string(forKey: "geminiModel") ?? "gemini-2.5-flash", key: key, glossary: UserDefaults.standard.string(forKey: "glossary") ?? "CAN, CAN FD, ECU, i.MX95, i.MX8MP, R818, LVDS, HDMI, MCU")
                            break
